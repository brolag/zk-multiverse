...
export async function generateAndVerifyProof2(a: number, b: number) {
  try {
    ...
    const { proof, publicSignals } = await groth16.fullProve(
      input,
      "/challenges/circom/challenge_2/files/Knightsknaves.wasm",
      "/challenges/circom/challenge_2/files/knightsknaves_0001.zkey",
    );

    const vKey = await fetch("/challenges/circom/challenge_2/files/verification_key.json").then(response =>
      response.json(),
    );
...
export async function generateAndVerifyProof3(a: number, b: number) {
  try {
    const input = {
      A_is_knight: a, // Ensure 'a' is either "Knight" or "Knave"
      B_is_knight: b, // Ensure 'b' is either "Knight" or "Knave"
    ...
    const { proof, publicSignals } = await plonk.fullProve(
      input,
      "/challenges/circom/challenge_3/files/abi.json",
...
export async function generateAndVerifyProof4(a: number, b: number) {
  try {
    ...
    const { proof, publicSignals } = await plonk.fullProve(
      input,
      "/challenges/circom/challenge_4/files/Knightsknaves.wasm",
      "/challenges/circom/challenge_4/files/knightsknaves_0000.zkey",
    );

    const vKey = await fetch("/challenges/circom/challenge_4/files/verification_key.json").then(response =>
      response.json(),
    );
    const isValid = await plonk.verify(vKey, publicSignals, proof);
